...
We will now create our workspace from a CodeReady _devfile_. This file contains, among other things, the instructions:
* which _stack_ (e.g. Java 8, Maven) our workspace will be based on
* which plugins we want to install
* which project will be cloned/imported
* which actions/commands we want to make available.

To create a new workspace from our _devfile_, click on the `Create a Custom Workspace` link in the upper right corner of the _Getting Started_ screen.

image:images/codeready-initial-screen.png[]

In the next screen, click on the `IMPORT DEVFILE` link:

image:images/codeready-create-custom-workspace.png[]

In the `URL` field, enter the following URL to our _devfile_: https://raw.githubusercontent.com/rhte2019-optaplanner/cloud-balancing-lab-crw/master/cloud-balancing-lab-crw-devfile.yaml
Click on the green `CREATE & OPEN` button.

image:images/codeready-devfile-create-and-open.png[]
...
image:images/codeready-maven-build.png[]
+
. Click on the `Run benchmarker` action to run a build with `benchmarker` profile activated.
